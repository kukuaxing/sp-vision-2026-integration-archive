...
public:
  ...
  // Iterated EKF update for strongly nonlinear rigid-body observations.  H is
  // relinearized at every iteration and scalar Huber weights limit the effect
  // of a bad corner/depth component without discarding the whole armor.
  Eigen::VectorXd update_iterated(
    ...
    std::function<Eigen::VectorXd(const Eigen::VectorXd &, const Eigen::VectorXd &)> x_subtract =
      [](const Eigen::VectorXd & a, const Eigen::VectorXd & b) { return a - b; },
    int max_iterations = 4, double stop_threshold = 1e-4,
