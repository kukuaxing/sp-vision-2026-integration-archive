  ...
  while (!exiter.exit()) {
    ...
    if (!targets.empty()) {
      ...
      if (tracked_x.size() >= 9) {
        ...
        gyro_model_fresh = xuc_gyro_center_follow_enabled &&
          tracker_converged && std::isfinite(gyro_center_yaw) &&
          std::isfinite(gyro_omega_rad_s) &&
          std::abs(gyro_omega_rad_s) >= xuc_gyro_omega_threshold_rad_s;
      }
    }
    if (gyro_model_fresh) {
    ...
    if (direct_yaw_guard_enabled) {
      ...
      if (valid_measurement) {
        ...
        command.shoot = false;
        if (gyro_center_follow_active) {
          // In small-gyro mode the visible plate bearing oscillates around the
          // vehicle. Following the EKF rotation center avoids switching the
          // gimbal command every time the next of four plates becomes visible.
          command.yaw = std::remainder(
            direct_yaw_filtered + direct_yaw_command_correction_rad,
            2.0 * M_PI);
        } else if (prediction_lead_limit_configured && native_prediction_valid) {
