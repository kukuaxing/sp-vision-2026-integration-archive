...
public:
	...
	const float Ki_comp = 0.5f;         // 积分系数（原2.0→0.5，降低积分绕线导致的停转冲击）
	const float Int_Limit = 3000.0f;     // 积分限幅
	const float Err_Threshold = 5.0f;    // 误差阈值
	const float Kff_vel = 0.2f;          // 大云台角速度前馈系数（抵抗摩擦拖拽）
	const float Kff_acc = 0.05f;         // 大云台角加速度前馈系数（抵抗惯性甩动）
	void Ontimer(uint8_t idata[][8], uint8_t* odata);
	float continuous_gimbal_yaw{};
	float absolute_target_auto{};
private:
...
public:
	...
	int32_t current{}, curspeed{}, inv_curspeed{}, setcurrent{},setspeed{}, torque_current, motor_status, motor_angle_status, sum_angle{};//这个current用于输出电流或者电压
	int16_t adjspeed{};
	...
	float setangle{}, angle[2]{},distance{}, initial_x{}, rota_angle{}, reset_rota_angle{}, delta_angle{},maxangle=75.f;
	float yaw_limit_center_ref = 0.0f;   // 以RESET模式记录的小yaw相对角为限位中心
	bool yaw_limit_center_valid = false;
	float Torque_left;
	float Torque_right;
	float const_dx = 0.004974;// m / s  /rpm
	float target_yaw_auto;
};
