...
#define now 0
#define last 1
static void buffer_append_int32(uint8_t* buffer, int32_t number, int16_t* index) { // [后续修改] 限定为文件内函数，避免全局符号冲突
	buffer[(*index)++] = number >> 24;
	buffer[(*index)++] = number >> 16;
	buffer[(*index)++] = number >> 8;
	buffer[(*index)++] = number;
}
static void buffer_append_int16(uint8_t* buffer, int16_t number, int16_t* index) { // [后续修改] 限定为文件内函数，避免全局符号冲突
	buffer[(*index)++] = number >> 8;
	buffer[(*index)++] = number;
}
...
DMMOTOR& DMMOTOR::State_Decode(CAN /*hcan*/, uint8_t idata[][8])//接收反馈数据
{
	...
	return *this; // [后续修改] 函数返回引用，避免non-void函数无返回值警告
}
...
void DMMOTOR::DMmotor_transmit(uint32_t id)
{
	...
	can1.Transmit(id + MOTOR_MODE, can1.jointpdata[id - 1], 8); // [后续修改] 达妙pitch电机挂在CAN1上
}

void DMMOTOR::DMmotorinit()
{
	CanComm_ControlCmd(can1, CMD_MOTOR_MODE, this->ID); // [后续修改] 使能帧发到电机ID(0x03)，数据0xFC=使能；位置速度模式由控制帧ID(0x100+id)决定
	delay.delay_ms(1);
}
...
void  DMMOTOR::CanComm_ControlCmd(CAN hcan, uint8_t cmd, uint32_t id)//使能帧
{
	...
	{
	case CMD_MOTOR_MODE:
		buf[7] = 0xFC;//使能电机（0xFC=使能；模式由控制帧ID偏移决定：0x100+id=位置速度模式）
		break;
